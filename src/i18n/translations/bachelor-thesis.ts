export const bachelorThesis = [
    ...
    },
    {
        params: {
            ...
            "whatisAR.answer1": "Асоціація виникає, коли кілька подій пов'язані між собою. Інтуїтивно можна описати асоціації " +
                "таким чином, що певні дані (або події, знайдені в даних) пов'язані з іншими даними або" +
                " подіями, що випливають з цих даних. Наприклад, дослідження, проведене в супермаркеті, може показати, що 65% відвідувачів, які купили" +
                 " картопляні чіпси, також придбають кока-колу, а якщо на такий набір діє знижка, то вони придбають її у 85% випадків.",
            "whatisAR.answer2": "Такі правила виявляють приховані зв'язки у, здавалося б, не пов'язаних між собою даних. Ці зв'язки називаються асоціативними правилами. Ті, що перевищують певний поріг, вважаються цікавими. " +
                "Такі правила дають можливість діяти на основі знайдених закономірностей. Вони також допомагають у прийнятті рішень і їх поясненні.",
            "class": "w-1/2 sm:w-1/5",
            "chips": "\\textrm{Чіпси}",
            "cocaCola": "\\textrm{Coca-Cola}",
...
export function getTranslationBachelorThesis(lang: string): any {
    const translation = bachelorThesis.find((t: { params: { lang: string; }; }) => t.params.lang === lang);
    if (translation) {
        return translation.params;
    }
    return bachelorThesis[0];
}